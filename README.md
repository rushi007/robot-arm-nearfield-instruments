# Robot Arm — Nearfield Instruments

Welcome to the **Robot Arm project** 

A Python-based implementation of a Robot Arm navigation system that processes movement instructions from an input file, 
validates them, and computes the resulting visited coordinates within a defined boundary.

This project demonstrates clean modular design, file-based execution, boundary validation, and automated testing using Pytest.

---

## 📂 Repository Structure

```text
robot-arm-nearfield-instruments/
│
├── app/
│   ├── __init__.py
│   ├── main.py                # Application entrypoint
│   └── file_parser.py         # Parses and validates system input file
│
├── robot_arm/
│   ├── __init__.py
│   └── robot_arm.py           # Core robot arm logic & movement handling
│
├── robot_arm_tests/
│   ├── helpers/
│   │   ├── __init__.py
│   │   └── file_parser.py     # Test-specific file utilities
│   │
│   ├── resources/             # Test input files
│   │   ├── system_input_all_points_within_bound.txt
│   │   ├── system_input_mix_coords.txt
│   │   ├── system_input_with_empty_points.txt
│   │   ├── system_input_with_invalid_boundry.txt
│   │   ├── system_input_with_invalid_boundry_ccords.txt
│   │   ├── system_input_with_outside_points.txt
│   │   ├── system_input_without_boundry_rectangle.txt
│   │   └── system_input_invalid_file_type.pdf
│   │
│   ├── output/                # Test execution outputs
│   │
│   ├── tests/
│   │   ├── __init__.py
│   │   ├── conftest.py        # Pytest fixtures
│   │   └── test_robot_arm.py  # End-to-end functional tests
│   │
│   └── __init__.py
│
├── system_output/             # Output files generated by application run
│
├── pytest.ini
├── requirements.txt
└── README.md
```

## Key Components

    app/ – Application entrypoint and input parsing

    robot_arm/ – Core robot movement and boundary handling logic

    robot_arm_tests/ – Test suite, fixtures, and test resources

    system_output/ – Output files generated during application execution

---

## 🚀 Features

✔ Modular architecture with separation of concerns 

✔ Deterministic coordinate tracking

✔ Input validation and boundary enforcement  

✔ End-to-end functional testing with Pytest


---

## 🧠 Getting Started

### 📦 Prerequisites

This project requires

**Python 3.8+**

**pip**

**Docker** (for viewing Allure reports)


It has been tested with standard Python interpreters on macOS.

Install the project dependencies:

```bash
pip install -r requirements.txt
```

## 🏁 Running the Application
The program expects exactly one input file containing:

- A boundary rectangle definition # For example: `(-4, -150), (-4, 150), (160, -150), (160, 150)`
- A list of navigation points # For example: `(-4, -150)`

### Sample Input File Format

```text
Rectangle
(-4, -150), (-4, 150), (160, -150), (160, 150)
Points
(-3, -149)
(4, 150)
(170, 150)
(150, -155)
```

### Run the application as follows:

```bash
python -m app.main <path_to_input_file>
```

For Example:

```bash
python -m app.main ./robot_arm_tests/resources/system_input_file.txt
```

## 🧪 Testing

The project includes a comprehensive test suite using Pytest to validate the functionality of the Robot Arm application.

### Test Structure
The test suite is organized into:
- `helpers/` – Contains utility functions for test setup and file handling
- `resources/` – Contains input files for various test scenarios
- `tests/` – Contains the actual test cases that validate the application behavior
- `output/` – Stores output files generated during test execution for verification
- `conftest.py` – Contains Pytest fixtures for setting up test environments and shared resources

### Scenarios covered by tests include:
- All points within the boundary rectangle
- A mix of points inside and outside the boundary
- Empty points section in the input file
- Invalid boundary definitions (e.g., non-rectangular, malformed coordinates)
- Points that are outside the defined boundary
- Missing boundary rectangle in the input file
- Invalid file types (e.g., non-text files)

### Running the Test Suite:

```bash
pytest
```

Or to run a specific test file:

```bash
pytest robot_arm_tests/tests/test_robot_arm.py
```

### Test Coverage and Results
To generate a test coverage report, run:

```bash
pytest --cov=robot_arm --cov=app
```

Or to generate an HTML coverage report:

```bash
pytest --cov=app --cov=robot_arm --cov-report=html
```

This will create an `htmlcov/` directory with a detailed coverage report.

open `htmlcov/index.html` in a web browser to view the report.

To see the lines for which coverage is missing,
open the specific file in the coverage report and look for the red highlights in the report.

Or run the following command to see the missing lines in the terminal:

```bash
pytest --cov=app --cov=robot_arm --cov-report=term-missing
```

## View Allure Report (Docker)

Run the below command to start the Allure Docker service, which will serve the Allure reports generated by the tests: 

(Run this command once in the terminal before running the tests)
```bash
docker run -d -p 5050:5050 \
-v ${PWD}/allure-results:/app/allure-results \
frankescobar/allure-docker-service
```

Then run the tests to generate the Allure results, 
the results will be saved in the `allure-results/` directory, 
and the Allure Docker service will automatically pick up the results and serve the report at `http://localhost:5050/allure-docker-service/latest-report`.
The allure extension in the `pytest.ini` file is configured to run with the default pytest command, so you can simply run:

```bash
pytest
```

Then open `http://localhost:5050/allure-docker-service/latest-report` in a web browser to view the Allure report.
